...
(function () {
  ...
  function openSpec(p) {
    ...
    try {
      if (window.druomuTrackPage) window.druomuTrackPage('/products/' + p.id, t(p.name));
    } catch (e) {}
  }
  ...
  function inquire(p) {
    ...
    if (msg) {
      ...
      /* 目录独立页：回主页联系表单并预填设备名 */
      try { sessionStorage.setItem('druomu_inquiry_msg', msgText); } catch (e) {}
      closeSpec();
  ...
  modal.querySelector('#spCta').addEventListener('click', function () {
    if (current) {
      /* 埋点：从产品详情发起询盘 = 强意向信号 */
      try {
        if (window.druomuTrack) window.druomuTrack('begin_inquiry', null, { product: current.name.en });
      } catch (e) {}
      inquire(current);
    }
  });
  ...
  function buildCatalog() {
    ...
    searchEl.addEventListener('input', function () {
      ...
      searchTimer = setTimeout(function () {
        ...
        try {
          if (window.druomuTrack) {
            window.druomuTrack('view_search_results', null, { search_term: q, results_count: hitCount });
          }
        } catch (e) {}
      }, 1200);
    });
  ...
  function boot() {
    ...
    if (searchEl) {
      ...
      if (qs) {
        ...
        try {
          if (window.druomuTrack) window.druomuTrack('select_category', null, { category: qs });
        } catch (e) {}
      }
    }
  }
  ...
  window.druomuQuickSearch = quickSearch;
})();
